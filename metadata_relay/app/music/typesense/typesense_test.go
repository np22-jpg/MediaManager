...
func TestTypesenseIntegration(t *testing.T) {
	...
	// Initialize Typesense from environment variables if available
	if host := os.Getenv("TYPESENSE_HOST"); host != "" {
		...
		if err := InitTypesense(host, port, apiKey, timeout); err != nil {
			t.Logf("Failed to initialize Typesense: %v", err)
		}
	}

	// Initialize MusicBrainz database connection if available
	if dbHost := os.Getenv("MUSICBRAINZ_DB_HOST"); dbHost != "" {
		dbName := os.Getenv("MUSICBRAINZ_DB_NAME")
		if dbName == "" {
			dbName = "musicbrainz_db"
		}
		dbUser := os.Getenv("MUSICBRAINZ_DB_USER")
		if dbUser == "" {
			dbUser = "musicbrainz"
		}
		dbPassword := os.Getenv("MUSICBRAINZ_DB_PASSWORD")

		connStr := os.Getenv("MUSICBRAINZ_DB_CONN_STR")
		if connStr == "" {
			connStr = "postgres://" + dbUser + ":" + dbPassword + "@" + dbHost + "/" + dbName + "?sslmode=disable"
		}

		// Initialize a temporary database connection for this test
		if testDB, err := sql.Open("postgres", connStr); err == nil {
			SetDB(testDB)
			defer func() {
				_ = testDB.Close()
			}()
